...
export async function chatHistoryUpdate(message: Message, data: Date){
	const historyChat = await db.findChatHistoryforTwo(message.senderId, message.receiverId);

	
	if(historyChat === 'NO HISTORY'){
		//Console.log('no history');
		const newChatHistory = ChatHistory.createHistoryChat(message.message, message.senderId, message.receiverId, data);
		...
		if(historyChat[0].users[0].userId === message.senderId){
			messageToSave = MessageToSave.createMessage(0, message.message, data);
		}
		else {
			messageToSave = MessageToSave.createMessage(1, message.message, data);
		}
